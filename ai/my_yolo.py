...
from ai.yolo3.model import yolo_eval, yolo_body, tiny_yolo_body
from ai.yolo3.utils import letterbox_image
import os
...
#--------------------AllGrocery DB 있을때----------------------
        ...
        #     tmp = self.class_names[c]
        #     names.append(tmp)
        #     objs = AllGrocery.objects.filter(name=tmp)
        #     for obj in objs:
        #         print(obj)
        #         all_grocery_id.append(obj.id)
        #
        # x_coords = (out_boxes[:, 0] + out_boxes[:, 2]) / 2
        # y_coords = (out_boxes[:, 1] + out_boxes[:, 3]) / 2
        #
        # df = pd.DataFrame({'all_grocery_id': all_grocery_id, 'name': names, 'x_coord': x_coords, 'y_coord': y_coords})
        # id_unq = df['all_grocery_id'].unique()
        # name_unq = df['name'].unique()
        # my_result = [{'all_grocery_id': id_unq[j], 'name': name_unq[j], 'count': 0, 'coordindate': []} for j in
        #              range(name_unq.shape[0])]
        #
        # for i in range(df.shape[0]):
        #     for j in range(len(my_result)):
        #         if my_result[j]['name'] == df.iloc[i, 0]:
